...
layui.define(['element','common','treefun'], function(exports) {
	"use strict";
	...
	Tab.prototype.tabAdd = function(data) {
		var that = this;
		var _config = that.config;
		var tabIndex = that.exists(data.name);
        var ck=$(".layui-nav-tree").find("a[data-url='"+data['href']+"']").parent().get(0);
        if(tabIndex === -1) {
			//设置只能同时打开多少个tab选项卡
			if(_config.maxSetting !== 'undefined') {
				...
				if(typeof _config.maxSetting === 'object') {
					var max = _config.maxSetting.max || 10;
			...
			//切换到当前打开的选项卡
			element.tabChange(ELEM.tabFilter, that.getTabId(data.name));
          //设置按钮可用
            btnShow($('iframe[data-id=' + globalTabIdIndex + ']').get(0),ck);
		} else {
			element.tabChange(ELEM.tabFilter, that.getTabId(data.name));
			//自动刷新
			...
			btnShow($('div.layui-tab-content > div').eq(tabIndex).children('iframe')[0],ck);
		}
        if(_config.contextMenu) {
			...
			ELEM.titleBox.find('li').on('contextmenu', function(e) {
				...
				var ul = '<ul>';
				ul += '<li data-target="refresh" title="刷新当前选项卡"> 刷新</li>';
				ul += '<li data-target="closeCurrent" title="关闭当前选项卡"> 关闭当前</li>';
				ul += '<li data-target="closeOther" title="关闭其他选项卡"> 关闭其他</li>';
				ul += '<li data-target="closeAll" title="关闭全部选项卡"> 全部关闭</li>';
				ul += '</ul>';
				div.innerHTML = ul;
						...
						$that.on('click', function() {
							...
							switch(target) {
								case 'refresh': //刷新当前
									...
									var src = ELEM.contentBox.find('iframe[data-id=' + id + ']')[0];
									if(!src){return;}else{src=src['src']}
									ELEM.contentBox.find('iframe[data-id=' + id + ']')[0].src = src;
									btnShow($('iframe[data-id=' + id + ']')[0],ck);
									break;
	...
	//页面按钮使用权限渲染
	function  btnShow(iframe,ck) {
	   var adminLevel=sessionStorage.getItem("adminLevel");
       iframe.addEventListener("load",function () {
           layer.close(loadIframe);
           if(adminLevel=="super"){
               $(iframe.contentDocument.getElementById("alist")).children("a").show();
               return;
           }else if(ck.nodeName=="DD"){
               var navs=treefun.stringifyArr(JSON.parse(adminLevel));
               var topIndex=$(ck).parent().parent().index();
               var zindex=$(ck).index();
               var childs=navs[topIndex]['children'];
               var btns=childs[zindex]['btns'];
               if(!!btns){
                   for(var i=0,leng=btns.length;i<leng;i++){
                       $(iframe.contentDocument.getElementById(btns[i]['ele'])).addClass('show').show();
                   }
               }
           }else{
               var navs=treefun.stringifyArr(JSON.parse(adminLevel));
               var topIndex=$(ck).index();
               var btns=navs[topIndex]['btns'];
               if(!!btns){
                   for(var i=0,leng=btns.length;i<leng;i++){
                       $(iframe.contentDocument.getElementById(btns[i]['ele'])).addClass('show').show();
                   }
           ...
           var a=$(iframe.contentDocument.getElementById("alist")).children("a.show").last();
           $(a).children("span.layui-box").remove();
       });
    }
